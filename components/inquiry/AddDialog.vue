<template>
  <form @submit.prevent="handleSubmit($event.target as HTMLFormElement)" class="w-full bg-white rounded-lg mx-auto">
    <h1 class="text-2xl font-bold text-center mb-6 text-gray-900">{{ $t('inquiry.addDialog.title') }}</h1>
    ...
    <section class="space-y-5 mb-8 w-full">
      ...
      <div>
        <label for="title" class="label-required block text-sm font-medium text-gray-700 mb-1">{{ $t('inquiry.addDialog.inquiryTitle') }}</label>
      ...
      <div>
        <label for="content" class="label-required block text-sm font-medium text-gray-700 mb-1">{{ $t('inquiry.addDialog.inquiryContent') }}</label>
    ...
    <div class="flex justify-center gap-3">
      <button type="button" class="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300" @click="$emit('close')">{{ $t('button.cancel') }}</button>
      <button type="submit" class="px-4 py-2 text-white bg-indigo-600 rounded-md md:hover:bg-indigo-700">{{ $t('button.submit') }}</button>
    </div>
  </form>
</template>
